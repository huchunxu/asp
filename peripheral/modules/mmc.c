...
#define FCLK_FAST()	 SD_SPI_VERY_HIGH_SPEED() /* Set fast clock (depends on the CSD) */
...
BYTE xchg_spi (BYTE dat)
{
    ...
    spi_snd(SPI_NO_GET(SD_SPI_NO), &dat, 1);
...
void rcvr_spi_m (BYTE *p)
{
    ...
    spi_snd(SPI_NO_GET(SD_SPI_NO), &data, 1);
        
    spi_rcv(SPI_NO_GET(SD_SPI_NO), p); 
}
...
void power_on (void)
{
    spi_init(SPI_NO_GET(SD_SPI_NO), MASTER);
}
...
)
{
	...
	if (ty) {			/* Initialization succeded */
		...
		//FCLK_FAST();
		spi_init_fast(SPI_NO_GET(SD_SPI_NO), MASTER);
