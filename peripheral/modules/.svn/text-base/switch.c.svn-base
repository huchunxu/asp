 ...
 *    @brief   开关初始化
 */
void switch_init(intptr_t exinf)
{
    uint_t i;
    SWCB *p_swcb;
    SWGP *p_swgp;
    ...
    for (p_swgp = swgp_table, i = 0; i < SWITCH_GROUP_NUM; p_swgp++, i++)
    {
        ...
        p_swgp->switch4 = SWITCH4;
        p_swgp->switch5 = SWITCH5;
        p_swgp->switch6 = SWITCH6;
        p_swgp->switch7 = SWITCH7;
    }
    
    switch_open_port(SWITCH_START0);
    switch_open_port(SWITCH_START1);

    switch_open_port(SWITCH0);
    switch_open_port(SWITCH1);
    switch_open_port(SWITCH2);
    switch_open_port(SWITCH3);
    switch_open_port(SWITCH4);
    switch_open_port(SWITCH5);
    switch_open_port(SWITCH6);
    switch_open_port(SWITCH7);
}
